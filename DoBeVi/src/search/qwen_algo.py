...
class QwenProver(Prover):
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("/data0/xs/LLM-ATP/model/llm_based_atp", trust_remote_code=True)
    ...
    async def _step(self, priority_queue: asyncio.PriorityQueue[Tuple[float, UnsolvedNode]]) -> None:
        ...
        if isinstance(search_node.leandojo_state, TacticState):
            prompt = self._build_prompt_qwen_few_shot_tactics(search_node)
        else:
    ...
    async def _generate_tactics(self, tactic_state_str: str) -> List[Tuple[str, float]]:
        ...
        # with open("tactics.txt", "a", encoding="utf-8") as f:
        #     for item in suggestions["suggestions"]:
        #         f.write(item["tactic"] + "\n\n")
        #         f.write("==================================\n\n")


        suggestions = self.extract_lean_tactics_with_score(suggestions["suggestions"])

        # suggestions = [(item['tactic'], item['score']) for item in extracted_suggestions ['suggestions']]
    ...
    def _build_prompt_qwen_zero_shot(
        self,
        search_node: UnsolvedNode,
    ) -> str:
        """Build a one-shot prompt for Qwen2.5-Math to predict the next Lean4 tactic."""
        prompt = [
            {
                "role": "system",
                "content": (
                    "You are a Lean 4 proof assistant. "
                    "You must predict exactly ONE valid Lean 4 tactic "
                    "that advances the proof toward completion.\n\n"
                    "### STRICT OUTPUT RULES:\n"
                    "- Do NOT output any explanations, reasoning, or commentary.\n"
                    "- Do NOT use markdown, backticks, or code fences.\n"
                    "- Begin your response IMMEDIATELY with <LEAN_TACTIC> on the first line.\n"
                    "- Then write exactly one tactic on the next line.\n"
                    "- Then close with </LEAN_TACTIC> on the following line.\n\n"
                    "### Examples:\n"
                    "<LEAN_TACTIC>\n"
                    "linarith\n"
                    "</LEAN_TACTIC>\n\n"
            ...
            },
            {
                "role": "user",
                "content": (
                    f"Current proof state:\n{search_node.leandojo_state.pp}\n\n"
                    "Now output the next Lean 4 tactic following the strict format above."
                )
            }
        ]
    ...
    def _build_prompt_qwen_few_shot(
        self,
        search_node: UnsolvedNode,
    ) -> str:
        """Build a few-shot prompt for Qwen2.5-Math to predict the next Lean4 tactic."""
        prompt = [
            {
                "role": "system",
                "content": (
    ...
    def _build_prompt_qwen_zero_shot_tactics(
        self,
        search_node: UnsolvedNode,
    ) -> str:
        """Build a one-shot prompt for Qwen2.5-Math to predict the next Lean4 tactic."""
        prompt = [
            {
                ...
                "content": (
                    ...
                    "### Common Lean 4 tactics (reference only — do NOT copy blindly):\n"
                    "- rfl: solves definitional equalities.\n"
                    "- simp: simplifies goals using rewriting rules.\n"
                    "- linarith: solves linear arithmetic.\n"
                    "- intro / intros: introduces hypotheses.\n"
                    "- apply h: applies lemma or hypothesis.\n"
                    "- rewrite [h] / rw [h]: rewrites goal using equality.\n"
                    "- exact h: directly closes goal.\n"
                    "- cases h: case split on an inductive hypothesis.\n"
                    "- refine / constructor: build a structure.\n"
                    "- have: introduce intermediate statement.\n"
            ...
            },
            {
                ...
                "content": (
                    f"Current proof state:\n{search_node.leandojo_state.pp}\n\n"
                    "Output the next Lean 4 tactic now."
                )
            }
        ]
        prompt = self.tokenizer.apply_chat_template(prompt, tokenize=False, add_generation_prompt=True)
    ...
    def _build_prompt_qwen_few_shot_tactics(
            self,
            search_node: UnsolvedNode,
        ) -> str:
            """Build a one-shot prompt for Qwen2.5-Math to predict the next Lean4 tactic."""
    ...
    def extract_lean_tactics_with_score(self, suggestions: List[dict]) -> List[Tuple[str, float]]:
        extracted = []
        pattern = re.compile(r"<LEAN_TACTIC>\s*(.*?)\s*</LEAN_TACTIC>", re.DOTALL)

        for item in suggestions:
            text = item.get("tactic", "")
            ...
            # ✅ 只取最后一个匹配结果
            tactic = matches[-1].strip()
            if tactic:
                extracted.append((tactic, score))

        print(f"✅ Extracted {len(extracted)} valid tactics out of {len(suggestions)} suggestions.")
        if extracted:
            for i, (tactic, score) in enumerate(extracted, 1):
                print(f"[{i}] {tactic}  (score={score})")
        return extracted
