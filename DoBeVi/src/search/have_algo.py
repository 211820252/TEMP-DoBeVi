...
class HaveProver(Prover):
    ...
    async def _best_first_search(self) -> None:
        ...
        self.use_have_initial = True

        while True:
            if self.use_have_initial:
                ...
                if self.num_expansions >= self.have_max_expansions or self.root.status == Status.INVALID:
                    self.use_have_initial = False
                    priority_queue.put_nowait((-self.root.priority, self.root))
            else:
                if priority_queue.empty():
                    logging.info("Search queue is empty.")
                ...
                if self.root.status == Status.SOLVED:
                    logging.info("Search complete: proof found.")
                    break
                elif self.num_expansions >= self.max_expansions:
                    logging.info("Exceeded time limit or max expansions.")
                    break
                elif self.root.status == Status.INVALID:
    ...
    async def _step(self, priority_queue: asyncio.PriorityQueue[Tuple[float, UnsolvedNode]]) -> None:
        ...
        suggestions = await self._generate_tactics(tactic_state_str, search_node is self.root)
    ...
    @torch.no_grad()
    async def _generate_tactics(self, tactic_state_str: str, use_have_model: bool) -> List[Tuple[str, float]]:
        start_time = time.time()
        suggestions = await client.async_generate_tactic_sampling(
            self.select_tac_gen(),
            state=tactic_state_str,
            num_samples=self.num_sampled_tactics if not use_have_model else 8,
        )
        suggestions = [(item['tactic'], item['score']) for item in suggestions['suggestions']] if "suggestions" in suggestions else []
        
        if use_have_model:
            have_suggestions = await client.async_generate_tactic_sampling(
                -1,
                state=tactic_state_str,
                num_samples=self.num_sampled_tactics,
            )
            have_suggestions = [(item['tactic'], item['score']) for item in have_suggestions['suggestions']] if "suggestions" in have_suggestions else []
            suggestions.extend(have_suggestions)
